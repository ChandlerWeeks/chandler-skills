...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript at `~/.claude/projects/<project-slug>/<session-id>.jsonl`, in the current project's directory only. Sibling project directories hold unrelated private chats. Walk the log against what actually happened:
