...
Before spawning investigators, list the MCP servers connected to this session. Their tools are named `mcp__<server>__<tool>`. Deferred tools appear by name only, so load their schemas with ToolSearch before classifying them.
...
- `subagent_type`: `Explore`. It is read-only and keeps MCP and Bash access, which the investigators need.
- `model`: `sonnet`
...
- `subagent_type`: `general-purpose`, so it keeps the MCP access its citation spot-check needs
- `model`: omit, so the subagent inherits the current model
- Read-only: tell the subagent to read and report, and to leave every file unchanged.
